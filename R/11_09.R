...
#'
#' @description Functions implementing the two optimization routines from previous classes (opt_mean1 and opt_mean2). These take a generic loss function "fn" as an argument.
#' @param x_range A \code{vector} representing the response or output.
#' @param fun A \code{string} of regressors or inputs.
#' @param B A \code{numeric} vector used for starting values (by default a vector of zeros).
#' @return A \code{list} minimum mean
#' @author Aya
